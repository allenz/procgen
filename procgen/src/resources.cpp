...
std::string colors_f = "colors.bin"; // generated by avg_colors.py
...
QColor get_color(std::string relpath) {
    if(colors.find(relpath) == colors.end()) {
        fatal("get_color: couldn't find %s in colors\n", relpath.c_str());
    }
    return colors.at(relpath);
}
...
void images_load() {
    ...
    std::ifstream f;
    f.open(global_resource_root + colors_f);
    char rgb[3];
    for(std::string sprite_path; f.read(rgb, 3) && std::getline(f, sprite_path); ) {
        colors[sprite_path] = QColor(((int)rgb[0]+256)%256, ((int)rgb[1]+256)%256, ((int)rgb[2]+256)%256);
    }
